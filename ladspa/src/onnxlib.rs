...
fn log_format(buf: &mut env_logger::fmt::Formatter, record: &log::Record) -> io::Result<()> {
    ...
    let level_style = buf.default_level_style(log::Level::Debug);
...
//                .filter_module("openvino", log::LevelFilter::Error)
...
impl Plugin for DfPlugin {
    ...
    fn run<'a>(&mut self, sample_count: usize, ports: &[&'a PortConnection<'a>]) {
        ...
        if rtf >= 1. {
            log::warn!(
                "DF {} | Underrun detected (RTF: {:.2}) sample_count: {:.2} sleep_duration {:.2} elapsed_time: {:.10} t_audio: {:.10} sr: {:.2}. Processing too slow!",
                self.id,
                rtf,
                sample_count,
